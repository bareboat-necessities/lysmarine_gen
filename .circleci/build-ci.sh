...
docker run --privileged --cap-add=ALL --security-opt="seccomp=unconfined" -d -ti -e "container=docker" -v "$WORK_DIR":rw -v /var/run/docker.sock:/var/run/docker.sock -v /dev:/dev "$DOCKER_IMAGE" /bin/bash
