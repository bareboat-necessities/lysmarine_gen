...
docker run --privileged --cap-add=ALL --security-opt="seccomp=unconfined" -d -i -e "container=docker" -v "$WORK_DIR":rw -v /dev:/dev "$DOCKER_IMAGE" /bin/bash
DOCKER_CONTAINER_ID=$(docker ps --last 4 | grep "$CONTAINER_DISTRO" | awk '{print $1}' | head -1)

docker exec --privileged -i "$DOCKER_CONTAINER_ID" apt-get update
docker exec --privileged -i "$DOCKER_CONTAINER_ID" apt-get -y install dpkg-dev debhelper devscripts equivs pkg-config apt-utils fakeroot \
  proot git-core live-build kpartx p7zip p7zip-full parted fdisk e2fsprogs qemu-user zerofree

docker exec --privileged -i "$DOCKER_CONTAINER_ID" /bin/bash -xec \
